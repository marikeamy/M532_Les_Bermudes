...
public class Use extends Command {
    ...
    public Use(String description, String verb, WorldMap map, Player player /*, Inventory inventory*/) {
        ...
        this.map = map;
        this.player = player;
        /*this.inventory = player.getInventory(); // On éviter de passer Inventory en paramètre puisqu’il est accessible via player.getInventory().*/
    }

    @Override
    public void execute(String argument) {
        if (argument == null || argument.isEmpty()) { //Cherche dans l’inventaire un objet dont le nom correspond à l’argument. Si aucun objet ne correspond, message d’erreur.
        ...
        // Si l’objet est une clé, on déverrouille l’endroit actuel via map.getPlayerLocation().unlock() on supprime l’objet de l’inventaire et affiche un message.
        if (itemToUse instanceof Key) {
            Key key = (Key) itemToUse;
            boolean success = key.unlockLocation();
            if (success) {
                inventory.remove(itemToUse);
                System.out.println("You used the " + key.getName() + " to unlock " + key.getLocationNameToUnlock() + ".");
            } else {
